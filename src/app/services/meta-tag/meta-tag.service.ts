...
export class MetaTagService {
  ...
  public setTags(pageData: PageData): void {
    ...
    // Image
    if (pageData.image) {
      this.metaService.addTag({ property: 'og:image', content: nicheShackUrl + pageData.image });
      this.metaService.addTag({ name: 'twitter:image', content: nicheShackUrl + pageData.image });
    }
  }
}